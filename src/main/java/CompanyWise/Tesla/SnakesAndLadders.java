...
or ladder is board[r][c]. Squares 1 and n2 are not the starting points of any snake or ladder.
...
public class SnakesAndLadders {
    ...
    void main() {
        ...
        int minDiceRolls = snakesAndLadders(board);

        System.out.println("minDiceRolls = " + minDiceRolls);
    }
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int dice = 1; dice <= 6; dice++) {
                    ...
                    // If snake or ladder exists, jump once
                    if (board[row][col] != -1) {
                        next = board[row][col];
                    }
